...
function renderVisualMuseum() {
  ...
  document.getElementById("visual-filled-count").textContent = `${filled}/83`;
  document.getElementById("visual-rating-points").textContent = total.toLocaleString();
  document.getElementById("visual-rating-label").textContent = rating;
  document.getElementById("visual-next-points").textContent = next ? (next.points - total).toLocaleString() : "0";
  document.getElementById("visual-progress-bar").style.width = `${progressValue}%`;

  const columns = Array.from({ length: 9 }, () => []);
  TIME_RIFT_PEDESTALS.forEach((pedestal, index) => {
    columns[index % columns.length].push(pedestal);
  });
  ...
  grid.innerHTML = columns.map((column) => `
    <div class="rift-column">
      ${column.map((pedestal) => {
        ...
        const score = calculateRelicScoreForPedestal(relic, pedestalType);
        const label = relic ? relic.name : `Slot ${pedestal.slot}`;

        return `
          <button class="rift-pedestal type-${pedestalType} ${relic ? "selected" : ""}" type="button" data-visual-slot="${pedestal.slot}" title="Slot ${pedestal.slot} · ${pedestalType}${relic ? ` · ${escapeHtml(relic.name)}` : ""}">
            <span class="rift-stars" aria-hidden="true">★★★★★★</span>
            <span class="rift-slot-score">${relic ? score : pedestal.slot}</span>
            <span class="rift-relic-name">${escapeHtml(label)}</span>
            <span class="rift-type-badge">${pedestalType}</span>
          </button>
        `;
      }).join("")}
    </div>
  ...
  document.querySelectorAll("[data-visual-slot]").forEach((button) => {
    button.addEventListener("click", () => {
      const slot = button.dataset.visualSlot;
      const filter = document.getElementById("slot-filter");
      if (filter) filter.value = "all";
      activeTab = "setup";
      renderAll();
      window.setTimeout(() => {
        const select = document.querySelector(`[data-relic-slot="${slot}"]`);
        select?.focus();
